...
party_radio_colours <- list(
  HTML("<p style='color:#d50000;'>Labour</p>"),
...
shapefile <- st_read(here(
  "data/London_Assembly_Constituencies_December_2018_Boundaries_EN_BFC",
...
run_assembly_election <- function(lab.seats,
  ...
  schedule <- tibble(
    party = party_names,
    constituency_seats = c(lab.seats, con.seats, lib.seats, 0, oth.seats),
  ...
  for (i in c(2:12)) {
    ...
    tt <- tt %>%
      mutate(
        round_vote = votes / (quota + seats_won),
        seats_won = case_when(
          round_vote == max(round_vote) ~ seats_won + 1,
